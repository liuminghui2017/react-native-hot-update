...
public class HotUpdateUtils {
    ...
    public static WritableArray convertJsonArrayToWritable(JSONArray jsonArr) {
        ...
        for (int i=0; i<jsonArr.length(); i++) {
            ...
            try {
                ...
                // Should not happen.
                throw new HotUpdateUnknownException(i + " should be within bounds of array " + jsonArr.toString(), jsonException);
            }
            ...
            if (obj instanceof JSONObject)
                ...
                throw new HotUpdateUnknownException("Unrecognized object: " + obj);
        }
    ...
    public static WritableMap convertJsonObjectToWritable(JSONObject jsonObj) {
        ...
        while(it.hasNext()){
            ...
            try {
                ...
                // Should not happen.
                throw new HotUpdateUnknownException("Key " + key + " should exist in " + jsonObj.toString() + ".", jsonException);
            }
                ...
                case Number:
                    ...
                    if ((number == Math.floor(number)) && !Double.isInfinite(number)) {
                        ...
                        try {
                            ...
                            throw new HotUpdateUnknownException("Unable to put value " + arr.getDouble(i) + " in JSONArray");
                        }
                    }
                    break;
                case Boolean:
            ...
            try {
                switch (type) {
                    ...
                    case Null:
                        ...
                        break;
                    default:
                        throw new HotUpdateUnknownException("Unrecognized type: " + type + " of key: " + key);
                }
            } catch (JSONException jsonException) {
                throw new HotUpdateUnknownException("Error setting key: " + key + " in JSONObject", jsonException);
            }
        }
    ...
    public static JSONObject getJsonObjectFromFile(String filePath) throws IOException {
        ...
        try {
            ...
            // Should not happen
            throw new HotUpdateMalformedDataException(filePath, jsonException);
        }
    }

    public static void log(String message) {
        Log.d(HotUpdateConstants.REACT_NATIVE_LOG_TAG, "[HotUpdate] " + message);
    }
    ...
    public static void setJSONValueForKey(JSONObject json, String key, Object value) {
        try {
            ...
            throw new HotUpdateUnknownException("Unable to set value " + value + " for key " + key + " to JSONObject");
        }
    }
